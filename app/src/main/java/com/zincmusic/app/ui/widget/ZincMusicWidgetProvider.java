package com.zincmusic.app.ui.widget;
...
/**
 * Base Widget Provider for Zinc Music.
 * Handles common playback control intents and updates all 3 widget sizes.
 */
public class ZincMusicWidgetProvider extends AppWidgetProvider {

    private static final String TAG = "ZincMusicWidget";

    // Intent Actions from AudioPlaybackService (must match exactly)
    private static final String ACTION_PLAY = "com.zincmusic.app.ACTION_PLAY";
    private static final String ACTION_PAUSE = "com.zincmusic.app.ACTION_PAUSE";
    private static final String ACTION_PREVIOUS = "com.zincmusic.app.ACTION_PREVIOUS";
    private static final String ACTION_NEXT = "com.zincmusic.app.ACTION_NEXT";
    private static final String ACTION_TOGGLE_LYRICS = "com.zincmusic.app.ACTION_TOGGLE_LYRICS";
    
    private static final String PREFS_NAME = "zincmusic_widget_prefs";
    ...
    private static void updateWidget(Context context, AppWidgetManager manager, int id, SongItem song,
        ...
        try {
            ...
            if (song != null) {
                ...
                if (layoutId == R.layout.widget_medium || layoutId == R.layout.widget_large) {
                    views.setTextViewText(R.id.widget_song_title, "Zinc Music");
    ...
    public static String findCurrentLyricLine(java.util.List<com.zincmusic.app.data.lyrics.LyricLine> lyrics,
            int position) {
        if (lyrics == null || lyrics.isEmpty())
            return null;

        String currentLine = lyrics.get(0).content;
        for (com.zincmusic.app.data.lyrics.LyricLine line : lyrics) {
    ...
    private static void setupPendingIntents(Context context, RemoteViews views, boolean isPlaying, int layoutId, Class<?> providerClass) {
        ...
        Intent launchIntent = new Intent(context, com.zincmusic.app.MainActivity.class);
        ...
        if (layoutId == R.layout.widget_large) {
            ...
            PendingIntent piFav = getServicePendingIntent(context, "com.zincmusic.app.ACTION_TOGGLE_FAV", 13);
            views.setOnClickPendingIntent(R.id.widget_favorite, piFav);

            // Repeat/Loop Button Intent
            PendingIntent piLoop = getServicePendingIntent(context, "com.zincmusic.app.ACTION_TOGGLE_LOOP", 14);
    ...
    private static PendingIntent getServicePendingIntent(Context context, String action, int requestCode) {
        Intent intent = new Intent(context, com.zincmusic.app.service.AudioPlaybackService.class);
