...
package com.zincmusic.app.service
...
import com.zincmusic.app.data.innertube.YouTubeMusic
import com.zincmusic.app.ui.widget.ZincMusicWidgetProvider
import com.zincmusic.app.util.AudioFileManager
import com.zincmusic.app.util.FFmpegCommandBuilder
...
class AudioPlaybackService : MediaSessionService() {
    ...
    companion object {
        ...
        const val ACTION_PLAY = "com.zincmusic.app.ACTION_PLAY"
        const val ACTION_PAUSE = "com.zincmusic.app.ACTION_PAUSE"
        const val ACTION_PREVIOUS = "com.zincmusic.app.ACTION_PREVIOUS"
        const val ACTION_NEXT = "com.zincmusic.app.ACTION_NEXT"
        const val ACTION_TOGGLE_LOOP = "com.zincmusic.app.ACTION_TOGGLE_LOOP"
        const val ACTION_CYCLE_PLAYBACK_MODE = "com.zincmusic.app.ACTION_CYCLE_PLAYBACK_MODE"
        const val ACTION_TOGGLE_FAV = "com.zincmusic.app.ACTION_TOGGLE_FAV"
    ...
    private var currentSongName = "Zinc Music"
    ...
    private lateinit var telemetryManager: com.zincmusic.app.data.innertube.YouTubeTelemetryManager
    ...
    override fun onCreate() {
        ...
        telemetryManager = com.zincmusic.app.data.innertube.YouTubeTelemetryManager(
            httpClient = com.zincmusic.app.data.innertube.InnerTubeClient.httpClient,
            clientProvider = { com.zincmusic.app.data.innertube.InnerTubeClient }
        )
                        ...
                        serviceScope.launch(Dispatchers.Main) {
                            viewModel?.let { vm ->
                                ...
                                val newSongs = related.map { onlineSong ->
                                    com.zincmusic.app.model.SongItem.createOnlineSong(
                                        onlineSong.videoId,
                                        onlineSong.title,
                                        onlineSong.artist,
    ...
    fun pause() {
        if (player.isPlaying) {
            ...
            com.zincmusic.app.util.PlaybackStateManager.savePosition(applicationContext, player.currentPosition)
        }
    }
    ...
    fun setSongMetadata(songName: String?, albumArt: Bitmap?) {
        this.currentSongName = songName ?: "Zinc Music"
                ...
                handler.post { 
                    ...
                    viewModel?.let { vm ->
                        if (vm.hapticManager == null) {
                            vm.hapticManager = com.zincmusic.app.util.PlayerHapticManager(applicationContext)
                        }
                    }
                }
            } catch (ex: Exception) {
            ...
            override fun run() {
                if (player.playbackState != Player.STATE_IDLE && player.playbackState != Player.STATE_ENDED) {
                    ...
                    if (player.isPlaying && (pos / 250) % 20L == 0L) {
                        com.zincmusic.app.util.PlaybackStateManager.savePosition(applicationContext, pos)
                    }
    ...
    private fun updateWidgetProgress(pos: Long, dur: Long) {
        ...
        viewModel?.syncedLyrics?.value?.let { lyrics ->
            if (lyrics.isNotEmpty()) {
                lyricText = ZincMusicWidgetProvider.findCurrentLyricLine(lyrics, pos.toInt()) ?: "..."
            }
        }
        ZincMusicWidgetProvider.updateAllWidgetsPartial(this, pos.toInt(), dur.toInt(), lyricText)
    }
    ...
    fun updateWidgetState(isPlaying: Boolean) {
        viewModel?.let { vm ->
            ...
            vm.syncedLyrics.value?.let { lyrics ->
                if (lyrics.isNotEmpty()) {
                    lyricText = ZincMusicWidgetProvider.findCurrentLyricLine(lyrics, pos) ?: "..."
                }
            }
            ZincMusicWidgetProvider.updateAllWidgets(this, song, isPlaying, pos, dur, lyricText)
        }
    }
    ...
    private fun setupMediaSession() {
        ...
        val intent = Intent(this, com.zincmusic.app.MainActivity::class.java)
    ...
    private fun setupPreferenceListener() {
        prefListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            ...
            if (com.zincmusic.app.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED == key || 
                com.zincmusic.app.ui.SettingsViewModel.KEY_TARGET_LUFS == key) {
                updateBaseVolume()
            }
        }
    }
    ...
    private fun updateBaseVolume() {
        val prefs = getSharedPreferences(com.zincmusic.app.ui.SettingsViewModel.PREFS_NAME,
            MODE_PRIVATE
        )
        val enabled = prefs.getBoolean(com.zincmusic.app.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED, false)
        ...
        val targetLufs = prefs.getFloat(com.zincmusic.app.ui.SettingsViewModel.KEY_TARGET_LUFS, -14f)
        ...
        if (song.lufs != null) {
            ...
            serviceScope.launch(Dispatchers.IO) {
                val measured = com.zincmusic.app.player.LoudnessAnalyzer.analyzeLufsFast(song.path)
                if (measured != null) {
                    // Update in-memory so we don't re-analyze
                    song.lufs = measured
                    // Save to playlist db if applicable
                    val db = com.zincmusic.app.data.db.MusicDatabase.getDatabase(applicationContext)
            ...
            serviceScope.launch(Dispatchers.IO) {
                val loudnessDb = com.zincmusic.app.data.innertube.YouTubeMusic.getLoudnessDb(song.videoId!!).getOrNull()
    ...
    private fun applyLufsGain(lufs: Float, targetLufs: Float) {
        val gain = com.zincmusic.app.player.VolumeNormalizer.calculateGain(lufs, targetLufs)
        val linearVolume = com.zincmusic.app.player.VolumeNormalizer.dbToLinear(gain)
