...
package com.zincmusic.app.data.innertube
...
object YouTubeMusic {
    ...
    fun prefetchStream(videoId: String?) {
        if (videoId.isNullOrEmpty()) return
        com.zincmusic.app.data.innertube.NewPipeStreamExtractor.prefetchBatch(
            listOf(videoId), 
            com.zincmusic.app.data.innertube.NewPipeStreamExtractor.PrefetchPriority.LOW
        )
    }
     ...
     */
    fun preCacheNextSong(song: com.zincmusic.app.model.SongItem) {
    ...
    @JvmStatic
    fun fetchAndAppendRelatedSongs(viewModel: com.zincmusic.app.viewmodel.PlayerSharedViewModel, videoId: String) {
        kotlinx.coroutines.CoroutineScope(Dispatchers.Main).launch {
            try {
                val result = relatedSongs(videoId)
                result.onSuccess { related ->
                    related.forEach { onlineSong ->
                        val songItem = com.zincmusic.app.model.SongItem.createOnlineSong(
                            onlineSong.videoId,
                            onlineSong.title,
                            onlineSong.artist,
